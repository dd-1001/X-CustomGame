...
local log = core.lib_logger("x-custom-game-electricity.lua")
...
-- 序列化table
local lib_serpent = require("__stdlib__/stdlib/vendor/serpent")
local serialization_format = { indent = "\t", maxlevel = 2 }

function show_data_raw(tab)
    log(lib_serpent.block(tab, serialization_format))
end
...
local tab_data_raw = data.raw["boiler"]["boiler"]
prot_type = "boiler"
prot_name = "boiler"
if mul and data.raw[prot_type][prot_name] then

    --show_data_raw(data.raw)
